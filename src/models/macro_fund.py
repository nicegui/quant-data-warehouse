...
class RawFundPosition(TimestampMixin, Base):
    """基金仓位估算 (日频, 三种类型) — legulegu.com via curl."""

    __tablename__ = "raw_fund_position"
    __table_args__ = (
        UniqueConstraint("trade_date", "fund_type", name="uq_fund_pos_date_type"),
        {"comment": "基金仓位估算(股票/灵活/平衡) — legulegu"},
    )

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    trade_date: Mapped[str] = mapped_column(String(16), nullable=False, index=True, comment="日期")
    fund_type: Mapped[str] = mapped_column(String(32), nullable=False, comment="基金类型 (stock/linghuo/pingheng)")
    position: Mapped[Optional[float]] = mapped_column(Float, nullable=True, comment="仓位百分比")
    close: Mapped[Optional[float]] = mapped_column(Float, nullable=True, comment="沪深300收盘价")
    raw_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)


class RawFundHolding(TimestampMixin, Base):
    """公募基金重仓持股 — akshare fund_portfolio_hold_em()."""

    __tablename__ = "raw_fund_holding"
    __table_args__ = (
        UniqueConstraint("fund_code", "stock_code", "quarter", name="uq_fh_fund_stock_qtr"),
        {"comment": "公募基金重仓持股 — akshare"},
    )
    ...
    stock_name: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, comment="股票名称")
    weight: Mapped[Optional[float]] = mapped_column(Float, nullable=True, comment="占净值比例(%)")
    ...
    quarter: Mapped[str] = mapped_column(String(32), nullable=False, comment="季度")
