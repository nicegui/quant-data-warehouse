"""同花顺概念板块资金流向 — MoneyflowCntThsCollector

Tushare moneyflow_cnt_ths API — 概念板块级别逐日资金流向。
~387 个概念板块/天，含领涨股 + 板块指数。
"""
...
from src.models.moneyflow import RawMoneyflowCntThs
...
class MoneyflowCntThsCollector(BaseTushareCollector):
    """同花顺概念板块资金流向 collector — 按交易日逐天回填."""

    def __init__(self, token: str):
        super().__init__("moneyflow_cnt_ths", token)
    ...
    def fetch(self, trade_date: str = "", ts_code: str = "",
        ...
        return self.api_call("moneyflow_cnt_ths", **params)
    ...
    def validate(self, raw: list[dict]) -> list[dict]:
        ...
        for row in raw:
            ...
            rec = {
                ...
                "industry_index": _f(row.get("industry_index")),
    ...
    def store_raw(self, records: list[dict]) -> int:
        ...
        with db_session() as session:
            for rec in records:
                existing = session.query(RawMoneyflowCntThs).filter_by(
                    trade_date=rec["trade_date"],
                    ts_code=rec["ts_code"],
                ).first()
                if existing:
                    continue
                session.add(RawMoneyflowCntThs(**rec))
                written += 1
        return written
    ...
    def _get_existing_dates(self) -> set[str]:
        try:
            ...
            rows = session.execute(
                text("SELECT DISTINCT trade_date FROM raw_moneyflow_cnt_ths")
            ).fetchall()
            session.close()
    ...
    def run(self, **kwargs) -> dict:
        ...
        d = datetime(2024, 10, 1)  # THS概念数据 ~2024-10 开始
        today = datetime.now()
        dp = []
        while d <= today:
            if d.weekday() < 5:
        ...
        elapsed = time.time() - t0
        logger.info("moneyflow_cnt_ths DONE: %d days, %s rows, %d skipped, %.0fs",
