"""
Pull raw_ggt_top10 only — all missing dates with rate limiting.
"""
import sys, time, os
...
with engine.connect() as conn:
    r = conn.execute(text("SELECT DISTINCT trade_date::date FROM raw_ggt_top10 ORDER BY 1"))
    existing = {row[0] for row in r.fetchall()}
    print(f"Existing ggt dates: {len(existing)}", flush=True)
...
if not missing:
    print("GGT already complete!")
    sys.exit(0)

total = len(missing)
rate_limit = 150  # per minute — stay under 200 limit
interval = 60.0 / rate_limit
inserted = 0
skipped = 0

cols = ['trade_date', 'ts_code', 'name', 'close', 'p_change', 'rank', 'market_type',
...
for i, d in enumerate(missing):
    ...
    for attempt in range(3):
        try:
            df = pro.ggt_top10(trade_date=date_str)
            if df is not None and len(df) > 0:
                ...
                with engine.begin() as conn:
                    for _, row in df.iterrows():
                        ...
                        conn.execute(
                            text(f"INSERT INTO raw_ggt_top10 ({col_names}) VALUES ({placeholders}) ON CONFLICT DO NOTHING"),
                            vals
                        )
                inserted += len(df)
            success = True
    ...
    if i % 200 == 0:
        with engine.connect() as conn:
            r = conn.execute(text("SELECT count(*) FROM raw_ggt_top10"))
            total_rows = r.scalar()
...
with engine.connect() as conn:
    r = conn.execute(text("SELECT count(*), min(trade_date), max(trade_date) FROM raw_ggt_top10"))
    cnt, mn, mx = r.fetchone()
    print(f"\nDONE! ggt_top10: {cnt} rows | {mn} ~ {mx}")
