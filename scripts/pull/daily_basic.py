#!/usr/bin/env python3
"""Pull daily_basic data.

Usage:
  python scripts/pull/daily_basic.py               # Latest trading day
  python scripts/pull/daily_basic.py --full         # All history (month by month)
  python scripts/pull/daily_basic.py --date YYYYMMDD
  python scripts/pull/daily_basic.py --start YYYYMMDD --end YYYYMMDD
"""
import sys
import os
import argparse
...
setup_logging()
log = get_logger("pull.daily_basic")


def get_collector():
    from src.collectors.impl.daily_basic import DailyBasicCollector

    return DailyBasicCollector(settings.tushare.token)
...
def pull_range(start: str, end: str):
    ...
    collector = get_collector()
    result = run_pipeline(
        collector, export=False, compute_curated=False, trade_date="", start_date=start, end_date=end
    )
    log.info(f"Range {start}-{end}: {result}")


def main():
    parser = argparse.ArgumentParser(description="Pull daily basic data")
